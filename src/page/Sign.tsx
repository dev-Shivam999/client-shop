...
const Sign = memo(() => {
    ...
    const hand = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (email.current?.value == "" && password.current?.value == "" && name.current?.value == "") {
            return alert("Please enter the email address and password and name")
            ...
            const data: AxiosResponse<LO> = await axios.post('https://sever-shop.onrender.com/user/Sign', {
                withCredentials: true,
            }, { data: { email: email.current?.value, password: password.current?.value, name: name.current?.value } })
        ...
        <div className="h-[80vh] py-6 flex flex-col justify-center sm:py-12">
            <div className=" lg:w-4/12 md:w-1/2 sm:w-6/12 w-full mx-auto">
                <div className="p-5 border-b-2">
                    <h4 className="font-semibold uppercase text-gray-700">Sign</h4>
                </div>
                <div className="p-5">
